...
export LinearPolarizer, WavePlate, QuarterWavePlate, HalfWavePlate, Rotator
...
"""
    LinearPolarizer(θ::Real=0.0)

Linear polarizer with transmission axis at angle θ (in radians).

# Arguments
- `θ::Real`: Angle of the transmission axis in radians (default: 0.0)
...
```
"""
function LinearPolarizer(θ::Real=0.0)
    c = cos(θ)
    s = sin(θ)
    M = [c^2 c*s; c*s s^2]
...
# Notes
The Jones matrix is constructed as: R(-θ) * [1 0; 0 exp(iΓ)] * R(θ)
where R(θ) is a rotation matrix.
"""
function WavePlate(Γ::Real, θ::Real=0.0)
    # Wave plate aligned at 0° (fast axis horizontal)
    M_plate = JonesMatrix([1.0 0.0; 0.0 exp(1im * Γ)])
